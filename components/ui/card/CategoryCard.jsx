...
export default function CategoryCard({
  ...
  const cardLayoutClass = currentGridCols === 1 ? "md:flex-row" : "flex-col";
  ...
  return (
    <Link href={`/products/${productId}`} className="block w-full">
      ...
      >
        ...
        {hasDiscount && (
          <div
            className={`absolute top-12 z-20 flex items-center justify-center rounded-full bg-red-700 p-2 text-xs font-bold text-white ${discountBadgePositionClass}`}
          >
            -{discount}%
          </div>
        )}
